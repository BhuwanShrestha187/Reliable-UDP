...
#include <unistd.h>
	...
	*/
	...
	/*
		* I made this changes: 
		* Add a local static variable to keep track of the packet count. 
	...
	*/
		...
		bool SendPacket(const unsigned char data[], int size)
		{
			...
			////Increment the packet count and create a custom messsage
			//
			//std::string customMessage = "Hello World <<" + std::to_string(packetCount++) + ">>";
			
			WriteHeader(packet, seq, ack, ack_bits);
			std::memcpy(packet + header, data, size); //Here also
		...
		int ReceivePacket(unsigned char data[], int size)
		{
			...
			std::memcpy(data, packet + header, static_cast<size_t>(received_bytes) - header);
